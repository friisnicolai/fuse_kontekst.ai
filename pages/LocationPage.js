...
var locationImageDisplay = location.map(function(x) { 
		return AppContext.mediaFilesLocation + x.location_image; 
	});
...
function refreshEntries() {
	...
	if(futureMode.value) {
		openkontekstapi.locGetFutureContentTimeEntries(location.value.location_id)
		.then(function(entriesList) {
			locations[location.value.location_id] = entriesList;
			numberOfEntries.value = entriesList.length;
			entries.replaceAll(locations[location.value.location_id]);	
			resetOffset();
			postInProgress.value = "Hidden";
		})
		.catch(function(error) {console.log("Error LocationPage.refreshEntries: " + JSON.stringify(error));});	
	}
	else if(pastMode.value) {
		openkontekstapi.locGetPastContentTimeEntries(location.value.location_id)
		.then(function(entriesList) {
			locations[location.value.location_id] = entriesList;
			numberOfEntries.value = entriesList.length;
			entries.replaceAll(locations[location.value.location_id]);	
		...
		openkontekstapi.locGetAllEntries(location.value.location_id)
		.then(function(entriesList) {
			locations[location.value.location_id] = entriesList;
			numberOfEntries.value = entriesList.length;
			entries.replaceAll(locations[location.value.location_id]);	
...
function postEntry() {
	...
	if(entryText.value.length > 0) {
		...
		GeoLocation.getLocation(5000).then(function(x) {
			console.log("Posting");
...
function activate() {
	if(lastLocation == "" || lastLocation != location.value.location_id) {
		entries.clear();
		refreshEntries();
		lastAgentlastLocation = location.value.location_id;
	}
...
function scrollToBottom() {
	locationPageScrollview.gotoRelative(1,1);
}


var pastTimeLocation = location.map(function(x) {
	if(x.past_time != null) { return "On";}
	else {return "Off";}
});

function togglePastTimeLocation() {
	if(location.value.past_time != null) {
		location.value.past_time = null;
	}
	else {
		location.value.past_time = utils.getCurrentDateTimeString();
	}
	openkontekstapi.updateLocationPastTime(location.value.location_id, location.value.past_time).then(function(x) {
		location.value = x;
	});
}
...
var futureTime = Observable(moment().format("DD.MM.YYYY HH:mm"));
...
function goToEditLocationsPage() {
	AppContext.addEditLocationCallback(function(x) {
		location.value = x;
	 });		
	router.push("editlocationpage", location.value);
}

module.exports = {
	location : location,
	locationImageDisplay : locationImageDisplay,
	entries : entries,
	...
	goToEditLocationsPage : goToEditLocationsPage,
	pastTimeLocation : pastTimeLocation,
	togglePastTimeLocation : togglePastTimeLocation
};